...
def BookResource_backend(_clubID, _resourceID, _date, _hr, _cost):
    nonce = w3.eth.get_transaction_count(settings.ACCOUNT_ADDRESS)
    txn = contract.functions.BookResource_backend(_clubID, _resourceID, _date, _hr, _cost).build_transaction(
    {
        'from': settings.ACCOUNT_ADDRESS,
        'gas': 200000,
        'gasPrice': w3.eth.gas_price,
